...
#program=ftvm
program=qemu
ssh mrhines@172.16.1.222 -f "sleep 5; DISPLAY=:${display}.0 vncviewer klinux9:5"
...
gdb /home/mrhines/$program/x86_64-softmmu/qemu-system-x86_64 -ex "$breakpoint1" -ex "$breakpoint2" -ex "$breakpoint3" -ex "$breakpoint4" -ex "handle SIGUSR2 noprint" -ex "handle SIGPIPE noprint" -ex "run -M pc-0.14 -enable-kvm -m 2048 -smp 2,sockets=1,cores=1,threads=1 -nodefconfig -rtc base=utc -no-shutdown /kvm_repo/vmbase_xen -serial pty -monitor unix:/tmp/devguest3,server,nowait -netdev type=tap,vhost=on,id=hostnet0,script=/home/mrhines/ftvm/kvmifup.sh,downscript=/home/mrhines/ftvm/kvmifdown.sh -device virtio-net-pci,netdev=hostnet0,mac=12:34:36:00:00:05 -usb -device usb-tablet,id=input0 -vnc 0.0.0.0:5 -nographic -device virtio-balloon-pci,id=balloon0,bus=pci.0,addr=0x5" 2>&1
